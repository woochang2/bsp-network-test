...
peer_names=("peer0" "peer1" "peer2" "peer3" "peer4")
peers=("localhost:7051" "localhost:7151" "localhost:7251" "localhost:7351" "localhost:7451")
...
function createOrg1 {
  ...
  OrdererOUIdentifier:
    ...
    for (( i = 0; i < ${#peers[@]}; i++ )); do
        ...
        echo
        set -x
        fabric-ca-client register --caname ca-org1 --id.name peer$idx --id.secret peer${idx}pw --id.type peer --tls.certfiles ${PWD}/organizations/fabric-ca/org1/tls-cert.pem
        set +x
    done
    ...
    for (( i = 0; i < ${#peers[@]}; i++ )); do
        ...
        idx=$(($i))
        # idx=$(($i+1))
        mkdir -p organizations/peerOrganizations/org1.example.com/peers/peer${idx}.org1.example.com
        echo
        echo "## Generate the peer$idx msp"
        echo
        set -x
        fabric-ca-client enroll -u https://peer$idx:peer${idx}pw@localhost:7054 --caname ca-org1 -M ${PWD}/organizations/peerOrganizations/org1.example.com/peers/peer$idx.org1.example.com/msp --csr.hosts peer$idx.org1.example.com --tls.certfiles ${PWD}/organizations/fabric-ca/org1/tls-cert.pem --csr.names O=Org1,OU=admin
        set +x
        ...
        echo
        set -x
        fabric-ca-client enroll -u https://peer$idx:peer${idx}pw@localhost:7054 --caname ca-org1 -M ${PWD}/organizations/peerOrganizations/org1.example.com/peers/peer$idx.org1.example.com/tls --enrollment.profile tls --csr.hosts peer$idx.org1.example.com --csr.hosts localhost --tls.certfiles ${PWD}/organizations/fabric-ca/org1/tls-cert.pem --csr.names O=Org1,OU=admin
        set +x
